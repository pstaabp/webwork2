...
any ['get', 'post'] => '/courses/:course_id/renderer/users/:user_id/sets/:set_id/problems/:problem_id' => sub {
  ...
  if((session 'permission') < 10){  ### check that the user belongs to the course and set.
    ...
    send_error("You are a student and must be assigned to the set $set_id.",404)
      unless (vars->{db}->existsUser($set_id) &&  vars->{db}->existsUserSet($user_id,$set_id));
  ...
  my $results = render(vars->{ce},vars->{db},$renderParams,\&debug);

  ## if it was a post request, then we record the the results in the log file and in the past_answer database
  if(request->is_post){
    ## need to derermine if it has checkAnswers or submitAnswers
    $results->{recorded_msg} = record_results(vars->{ce},vars->{db},$renderParams,$results,\&debug);
  }
