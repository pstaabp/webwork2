  ...
  <b-modal size="xl" id="import-students-file" ref="sfmodal" title="Import Students from a File">
      ...
      <b-row>
        ...
        <b-col>
          <b-input-group>
            ...
            <b-checkbox class="ml-3" v-model="ignoreFirstRow" @change="useFirstRow">Use first row as header</b-checkbox>
          </b-input-group>
        </b-col>
      </b-row>
      <b-row v-if="data != null">
        <b-col v-for="i in numCols" :key="i">
          <b-select :options="headers" v-model="headerValues[i-1]" size="sm"/>
        </b-col>
      </b-row>
      <b-row v-if="data != null">
    ...
    <div slot="modal-footer" class="w-100">
      <b-btn-group size="sm" class="float-right">
        <b-btn variant="outline-dark" @click="$bvModal.hide('import-students-file')">Cancel</b-btn>
...
export default class ImportStudentsFile extends Vue {
  ...
  private addAll() {
    ...
    const _users = this.data.map( (_u: any) =>
        this.headerValues.reduce((result: { [index: string]: {message: string} }, _val, i) => {
