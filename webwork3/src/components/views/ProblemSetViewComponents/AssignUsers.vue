<template>
  <b-container>
      ...
      <b-col cols="4">
        <b-form-group>
          <b-form-checkbox-group v-model="show_info">
            ...
            <b-form-checkbox value="recitation">Show Recitation</b-form-checkbox>
          </b-form-checkbox-group>
        </b-form-group>
      </b-col>
      <b-col cols="2" v-if="list_type === 'unassigned'">
        <b-btn variant="outline-dark" size="sm" @click="assignUsers">Assign to Selected Users</b-btn>
      </b-col>
      <b-col cols="2" v-else>
        <b-btn variant="outline-dark" size="sm" @click="save">Save Override Dates</b-btn>
      </b-col>
    </b-row>
    <b-row v-if="list_type === 'assigned'">
      <b-col cols="3">
        <b-form-group label="Open Date" class="header">
          <b-input size="sm" type="datetime-local" :value="override_dates.open_date | formatDateTime"
      ...
      <b-col cols="3">
        <b-form-group class="header" label="Reduced Scoring Date"
            ...
            <b-input size="sm" :value="override_dates.reduced_scoring_date | formatDateTime"
              :state="validReducedScoring" type="datetime-local"
      ...
      <b-col cols="3">
        <b-form-group label="Due Date" class="header"
              ...
              <b-input size="sm" :value="override_dates.due_date | formatDateTime"
                :state="validDueDate" type="datetime-local"
      ...
      <b-col cols="3">
        <b-form-group class="header" label="Answer Date"
              ...
              <b-form-input size="sm" :value="override_dates.answer_date | formatDateTime"
    ...
    <b-row>
      <b-col>
        <b-table small :items="users" :fields="fields" selectable
          select-mode="range" @row-selected="rowSelected" :filter="filter_text"
    ...
    <b-row>
      <b-col>
        <b-pagination v-model="current_page" limit="10" :total-rows="users.length" :per-page="per_page" />
      </b-col>
    </b-row>
  </b-container>
</template>
...
export default class AssignUsers extends mixins(ProblemSetMixin) {
  ...
  private setUsers(): void {
    const assigned_users_ids = this.problem_set && this.problem_set.assigned_users ?
                                  this.problem_set.assigned_users : [];
    const all_user_ids = Array.from(users_store.users.keys());
    const unassigned_users_ids = all_user_ids.filter( (_id) => !assigned_users_ids.includes(_id));

    if (this.list_type === 'assigned') {
      this.users = Array.from(users_store.users.values())
          .filter( (_u) => assigned_users_ids.includes(_u.user_id));
    } else if (this.list_type === 'unassigned') {
      this.users = Array.from(users_store.users.values())
          .filter( (_u) => unassigned_users_ids.includes(_u.user_id));
    } else {
      this.users =  Array.from(users_store.users.values());
    }
  }
  ...
  private setSelectedCheckboxes() {
    ...
    const users = users_store.users_array
              .filter((_u: User, i: number) => i < this.current_page * this.per_page &&
