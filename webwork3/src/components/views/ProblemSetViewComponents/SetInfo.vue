<template>
  <table class="table table-sm set-info-table" v-if="problem_set.set_id !=='XXX'">
    <tbody>
      <tr>
        <td class="header">Set Name</td><td>{{problem_set.set_id.replace('_', ' ')}}</td>
      </tr>
      <tr>
        <td><b-form-group label="Open Date" class="header">
          <b-input size="sm" type="datetime-local" :value="problem_set.open_date | formatDateTime"
        ...
        <td>
          <b-form-group class="header" label="Reduced Scoring Date"
            ...
            <b-input size="sm" :value="problem_set.reduced_scoring_date | formatDateTime"
      ...
      <tr>
        <td>
          <b-form-group label="Due Date" class="header"
            ...
            <b-input size="sm" :value="problem_set.due_date | formatDateTime"
        ...
        <td><b-form-group class="header" label="Answer Date"
            ...
            <b-form-input size="sm" :value="problem_set.answer_date | formatDateTime"
      ...
      <tr><td class="header">Set Type</td>
        <td><b-form-select size="sm" v-model="problem_set.assignment_type">
...
export default class SetInfo extends mixins(MessagesMixin, ProblemSetMixin) {
  ...
  get problemsLength(): number {
    return (this.problem_set && this.problem_set.problems) ? this.problem_set.problems.length : 0;
  }
  ...
  get gateway(): boolean {
    return this.problem_set.assignment_type === 'gateway' ||
  ...
  private problemSetChanged(new_set: ProblemSet, old_set: ProblemSet) {
    if (new_set && old_set && new_set.set_id === old_set.set_id) {
      if (Common.validDueDate(this.problem_set) && Common.validReducedScoring(this.problem_set)
