...
export default class CalendarRow extends Vue {
  ...
  private assignmentOnDate(day: moment.Moment): AssignmentInfo[] {
    const allDates = Array.from(this.problem_sets.values())
      .flatMap(_set => [
        {
    ...
    return allDates.filter((_date: AssignmentInfo) =>
      _date.date.isSame(day, "day")
    );
  }

  private assignChange(
    newDate: moment.Moment,
    evt: ChangeEvent<{ date: moment.Moment; type: string; set_id: string }>
  ) {
