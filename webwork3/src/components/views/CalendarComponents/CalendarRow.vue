...
export default class CalendarRow extends Vue {
  ...
  private get assignment_info(): AssignmentInfo [] {
    ...
    console.log("in get assignment_info");
    return this.week.map( (_day) => this.all_assignment_dates.filter( (_info: AssignmentInfo) =>
  ...
  private changin() {
    // tslint:disable-next-line
    console.log("assignment_info changed");
    // tslint:disable-next-line
    console.log(this.assignment_info);
  }
  ...
  private assignChange(newDate: moment.Moment, evt: MoveEvent<any>) {
    ...
    if (evt.hasOwnProperty('added')) {
      ...
      if (evt.added.element.date) { // it was dragged from the calendar
      ...
      // tslint:disable-next-line
      console.log(attrs);
      const tmp = Object.keys(attrs).reduce( (obj,value) => {obj[value] = moment.unix(attrs[value]).format("MM/DD/YYYY"); return obj;},{});
      // tslint:disable-next-line
      console.log(tmp);
