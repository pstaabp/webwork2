...
export default class MessagesModule extends VuexModule {
  ...
  // Messages
  @Action
  public addMessage(_msg: Message): void {
    _msg.id = this.messages_array.length;
    this.ADD_MESSAGE(_msg);
  }
  @Action
  public removeMessageById(_id: number): void {
  ...
  @Mutation
  private ADD_MESSAGE(_msg: Message): void {
    this.messages_array.push(_msg);
  }

  @Mutation
  private REMOVE_MESSAGE_BY_INDEX(_index: number): void {
