...
subtest 'check for admin require_role' => sub {
  ## test the user_roles
  my $req = GET "$url/courses/admin/test-for-admin";
  $jar->add_cookie_header($req);
  my $res = $test->request($req);

  ok($res->is_success, 'check that require_role is successful');

};

subtest 'get course parameters' => sub{
  my $req = GET "$url/admin/courses/new_course_xyz";
  $jar->add_cookie_header($req);
  my $res = $test->request($req);
...
subtest 'rename a course' => sub {
  ...
  my $req = HTTP::Request->new("PUT","$url/admin/courses/new_course_xyz",
    HTTP::Headers->new('Content-Type' => 'application/json'),
    encode_json($params)
  );
  $jar->add_cookie_header($req);
