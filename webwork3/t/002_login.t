...
#   $req =  GET "$url/courses/test/login?username=dave&password=dave";
...
subtest 'Check login with body parameters' => sub {
  ...
  my $params = {user_id => "dave",  password=> "dave"};
  my $res = $test->request(POST "$url/courses/test/login",
...
subtest 'Check the require_login' => sub {
  ...
  $res = $test->request(POST "$url/courses/test/login?user_id=peter&password=peter");
  ...
  $res = $test->request(POST "$url/courses/admin/login?user_id=admin&password=admin");
  my $jar3  = HTTP::Cookies->new();
  $jar3->extract_cookies($res);

  $req = GET "$url/courses/admin/test-for-admin";
  $jar3->add_cookie_header($req);
  $res = $test->request($req);

  ok($res->is_success, 'require_role for admin is working');
