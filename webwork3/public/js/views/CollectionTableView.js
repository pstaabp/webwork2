...
Notes:
The values of each model are stored in a _values field.  This way things can be easily sorted on or searched for.
These values can be separate from the visible items in the table.
  ...
  var CollectionTableView = Backbone.View.extend({
    ...
    initialize: function (options) {
      ...
      if(typeof(this.collection)==="undefined"){
        console.error("The attribute collection must be passed.")
      }
      if(! (this.collection instanceof Backbone.Collection)){
        console.error("The collection must be of type Backbone.Collection");
      }
      if(typeof(options.columnInfo)==="undefined"){
        console.error("The attribute columnInfo must be passed");
      }
    ...
    updateModelValues: function(model,_model){
      ...
      _(this.columnInfo).each(function(col){
        if(_.isFunction(col.value)){
          ...
          model.set(col.key+"_value",_model.get(col.key),{silent: true});
        }
        if(col.searchable){
